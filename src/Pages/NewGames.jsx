...
import NewGamesSidebar from '../Components/NewGamesSidebar'
...
const NewGames = () => {
    ...
    const getNewGames = async () => {
        setLoading(true)
        try {
            const response = await fetch(`${process.env.REACT_APP_BASE_URL}/products/newgames/game&new`, {
                method: "GET"
            })
        ...
        <div className='homepage flex flex-col justify-between min-h-screen'>
            ...
            <div className='main-container w-[100%] self-center flex flex-col justify-center '>
                <h1 className='page-title text-center text-3xl font-semibold self-center my-5 pb-5 w-[70%]'
                    style={{ borderBottom: "1px solid #eaeaea" }}>Giochi Nuovi</h1>
                <NewGamesSidebar
                    setRenderedNewGames={setRenderedGames}
                    newGames={games}
                />
                ...
                <div className='card-container self-center flex flex-wrap justify-center items-start'>
                    ...
                    {!loading && error && <p className='fetch-error text-red'>{error}</p>}
